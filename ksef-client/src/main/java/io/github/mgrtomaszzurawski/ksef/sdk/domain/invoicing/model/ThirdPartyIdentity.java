...
/**
 * Identifying data of a third party on an invoice — the typed view of a
 * {@code TPodmiot3} XSD complex type ({@code Podmiot3/DaneIdentyfikacyjne}).
 *
 * <p>The tax identifier is an XSD choice: exactly one of a Polish NIP
 * ({@link #nip()}), a NIP-derived internal identifier ({@link #internalId()}
 * = {@code IDWew}), an EU VAT number ({@link #euVatPrefix()} +
 * {@link #euVatNumber()}), another tax identifier with its issuing country
 * ({@link #taxIdCountryCode()} + {@link #otherTaxId()}), or the
 * "no identifier" marker ({@link #noTaxId()}). All members are therefore
 * nullable; the branches that do not apply are null. {@link #name()} is
 * optional in every branch.
 *
 * @param nip Polish tax identifier ({@code NIP}); null unless the NIP branch applies
 * @param internalId NIP-derived internal identifier ({@code IDWew}); null unless that branch applies
 ...
 * @since 0.1.0
 */
public record ThirdPartyIdentity(
        @Nullable String nip,
